...
var TokenLocker = TokenLocker || (function () {
    ...
    commandHelp = function(msg) {
        ...
        message += '<hr><h4>Locked Token Marker</h4>' + getMarker(curr_marker.tag, marker_style) + 'This is the current Locked token marker. You may change it below.';
    ...
    commandShowMarkers = function () {
        ...
        _.each(ALT_MARKERS, function (marker) {
            ...
            if (marker == state['TokenLocker'].marker) {
                message += '<td style="text-align: center;">Current</td>';
            } else {
                message += '<td style="text-align: center; white-space: nowrap; padding: 7px;"><a style="' + styles.button + '" href="!tl set-marker ' + marker.tag + '">Set Marker</a></td>';
            }
            message += '</tr>';
        });
        ...
        _.each(MARKERS, function (icon) {
            ...
            if (icon.tag == state['TokenLocker'].marker) {
                ...
                message += '<td style="text-align: center; white-space: nowrap; padding: 7px;"><a style="' + styles.button + '" href="!tl set-marker ' + icon.tag.replace('::','=') + '">Set Marker</a></td>';
            }
            message += '</tr>';
        });
